...
def discount_rewards(r, gamma, reward_to_go):
...
def train_PG(exp_name='',
    ...
    if discrete:
        ...
        # YOUR_CODE_HERE
        sy_mean = build_mlp(sy_ob_no, ac_dim, "policy", n_layers, size)
        sy_logstd = tf.Variable(tf.zeros([1,sy_mean]), name="logstd", dtype=tf.float32) # logstd should just be a trainable variable, not a network output.
        sy_std = tf.exp(sy_logstd)

        sy_sampled_ac = sy_mean + sy_std * tf.random_normal(tf.shape(sy_mean))

        sy_z = (sy_ac_na - sy_mean) / sy_std  # Hint: Use the log probability under a multivariate gaussian. 
    ...
    for itr in range(n_iter):
        ...
        q_n = []
        for path in paths:
            q_n.extend(discount_rewards(path["reward"], gamma, reward_to_go))
        ...
        if nn_baseline:
            ...
            sess.run([baseline_update_op, baseline_loss], feed_dict={sy_ob_no : ob_no, sy_target_n : q_normalized_n})
        ...
        # YOUR_CODE_HERE
        sess.run([update_op, loss], feed_dict={sy_ob_no : ob_no, sy_ac_na : ac_na, sy_adv_n : adv_n})
